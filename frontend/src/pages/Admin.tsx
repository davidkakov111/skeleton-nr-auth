...
import { DataGrid, getGridDateOperators, getGridNumericOperators, getGridSingleSelectOperators, type GridColDef } from '@mui/x-data-grid';
...
export default function Admin() {
    ...
    const handleRoleChange = async (userId: number, role: UserRole) => {
        ...
        try {
            ...
            if (status == 403) {
                ...
                enqueueSnackbar("Couldn't update user role due to invalid credentials", { variant: "warning" });
                console.warn("Missing ID and/or role, or role is invalid: ", err);
            } else if (status === 404) {
                enqueueSnackbar("Couldn't update user role because the user was not found", { variant: "warning" });
                console.warn("Couldn't update user role because the user was not found: ", err);
            } else {
                enqueueSnackbar("Couldn't update the user role due to an unexpected error.", { variant: "error" });
                console.error("Couldn't update the user role due to an unexpected error: ", err);
            }
        } finally {
        ...
        allUsers()
            ...
            .catch(err => {
                ...
                if (status == 403) {
                    ...
                    enqueueSnackbar("Couldn't fetch users due to an unexpected error.", { variant: "error" });
                    console.error("An unexpected error occurred in allUsers API call: ", err);
                }
            })
            .finally(() => {
                setLoading(false);
    ...
    const columns: GridColDef[] = [
        { field: 'id', headerName: 'ID', flex: 1, filterOperators: getGridNumericOperators() },
        { field: 'email', headerName: 'Email', flex: 1 },
        { field: 'role', headerName: 'Role', flex: 1, type: 'singleSelect', valueOptions: [...USER_ROLES],
        ...
        { field: 'createdAt', headerName: 'Created At', flex: 1, filterOperators: getGridDateOperators(), 
            valueGetter: (params) => new Date(params), valueFormatter: (params) => new Date(params).toLocaleString()
        },
    ];
